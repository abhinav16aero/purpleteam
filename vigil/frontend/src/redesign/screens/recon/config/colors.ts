/* Node colors by type — semantic color mapping, ported verbatim from
   webapp/src/app/graph/config/colors.ts, extended with the coordinator KG
   node kinds (host/service/finding/technique/credential/...) so the purple-team
   graph reads in the same palette as the webapp's recon map. */
...
export const NODE_COLORS: Record<string, string> = {
  ...
  // ── Purple-Team KG kinds (coordinator Neo4j graph) ──
  Host: '#2563eb',            // Royal blue - the asset red reached
  Finding: '#f97316',         // Orange - a red finding, tinted by detection below
  Technique: '#eab308',       // Yellow - MITRE technique node (T-id)
  Mitre: '#f97316',           // Orange - MITRE data
  Credential: '#e11d48',      // Rose - found credentials
  Account: '#c026d3',         // Fuchsia - accounts
  Agent: '#a855f7',           // Purple - coordinator/red agents
  Evidence: '#22c55e',        // Green - chained evidence ledger
  Event: '#78716c',           // Stone gray - timeline events
  Asset: '#0d9488',           // Teal - generic assets
  Node: '#6b7280',            // Gray - unlabeled nodes

  Default: '#6b7280',        // Gray - Fallback
}

/* Detection-based tinting for the purple-team graph: a technique (or a finding
   that USES one) is colored by whether the blue side detected it. */
export const DETECTION_COLORS = {
  detected: '#22c55e',  // green - blue caught it
  missed: '#ef4444',    // red - blue missed it
} as const

export const DETECTION_TINT_TYPES = new Set(['Technique', 'Mitre', 'Finding'])
