    ...
    <div className="ra-controls">
      ...
      {is3D ? (
        <div className="ra-controls-row">
          <button
            type="button"
            className={`ra-controls-btn${mode === 'translate' ? ' ra-controls-btn-active' : ''}`}
            onClick={() => setMode('translate')}
            title="Move mode"
          >
            <Move size={ICON_SIZE} />
          </button>
          <button type="button" className="ra-controls-btn" {...onDown(() => dpad(0, 1))} title={mode === 'rotate' ? 'Tilt up' : 'Pan up'}>
            <ChevronUp size={ICON_SIZE} />
          </button>
          <button
            type="button"
            className={`ra-controls-btn${mode === 'rotate' ? ' ra-controls-btn-active' : ''}`}
        ...
        <div className="ra-controls-row">
          <button type="button" className="ra-controls-btn" {...onDown(() => zoom(-1))} title="Zoom in">
            <ZoomIn size={ICON_SIZE} />
          </button>
          <button type="button" className="ra-controls-btn" {...onDown(() => dpad(0, 1))} title="Pan up">
            <ChevronUp size={ICON_SIZE} />
          </button>
          <button type="button" className="ra-controls-btn" {...onDown(() => zoom(1))} title="Zoom out">
      ...
      <div className="ra-controls-row">
        <button type="button" className="ra-controls-btn" {...onDown(() => dpad(-1, 0))} title={is3D && mode === 'rotate' ? 'Orbit left' : 'Pan left'}>
          <ChevronLeft size={ICON_SIZE} />
        </button>
        <button type="button" className="ra-controls-center-btn" onClick={resetView} title="Fit all nodes">
          <Crosshair size={ICON_SIZE} />
        </button>
        <button type="button" className="ra-controls-btn" {...onDown(() => dpad(1, 0))} title={is3D && mode === 'rotate' ? 'Orbit right' : 'Pan right'}>
      ...
      {is3D ? (
        <div className="ra-controls-row">
          ...
          <button type="button" className="ra-controls-btn" {...onDown(() => dpad(0, -1))} title={mode === 'rotate' ? 'Tilt down' : 'Pan down'}>
        ...
        <div className="ra-controls-row">
          <div className="ra-controls-spacer" />
          <button type="button" className="ra-controls-btn" {...onDown(() => dpad(0, -1))} title="Pan down">
            <ChevronDown size={ICON_SIZE} />
          </button>
          <div className="ra-controls-spacer" />
        </div>
