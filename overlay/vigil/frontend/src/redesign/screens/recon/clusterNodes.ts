 ...
 * synthetic cluster nodes. Cluster id: `cluster:<parentId>:<childType>`.
 */
 ...
 * anchor. Group such nodes by the anchor + type, collapse if group size ≥ threshold.
...
/**
 * Main entry: run the clustering pipeline (adaptive-threshold leaf clustering
 * cascaded until stable, then structural-edge clustering, then one more cascade).
 */
export function clusterGraphData(
  ...
  // Pass 1: cascading leaf clustering.
  let result = data
  for (let i = 0; i < 5; i++) {
